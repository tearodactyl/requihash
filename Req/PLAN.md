...
| A7 | x86-64/AVX2 leg of A4b | **Deprioritized; premise partially changed (2026-07-15)** | Dropped per direct instruction; re-open only if an x86 box and a decision-relevant AVX2 question both appear. Premise update, not a reopening: RT's investigation (`SOLVER_CORPUS.md` RT section) confirmed `clang -target x86_64-apple-macos` + Rosetta 2 produces and runs real x86_64 binaries on this machine — so "no x86 box" is no longer strictly true for *correctness*/functional testing of an AVX2 code path. It remains true for *performance* comparison: Rosetta's binary-translation overhead is unmeasured and would make a blake2b-AVX2-vs-blake2b-scalar timing claim untrustworthy without first establishing Rosetta's own overhead as a control — a real x86 box (or cloud instance) is still needed before A7's original timing question is answerable, so this stays deprioritized, not reopened |
...
| A13 | BLAKE2b NEON backend (aarch64) | **Demoted to icebox (2026-07-16) — premature** | Full analysis: `BLAKE/BLAKE.md` §5.2. BLAKE2b is intrinsically NEON-weak (64-bit words → 2 lanes, no native 64-bit rotate; the package's own `neon/` code has slower-than-scalar reports), hashing is ~17% of solve time (BENCHMARK §4), `blake2b_simd`'s portable path already serves ARM, and no consumer demands it. If ARM hash throughput ever matters, the better levers are batching or BLAKE3 (real NEON), not a BLAKE2b NEON port. Clone still available under `~/Work/ZK/ZKs/BLAKE/blake2-reference/neon/` | Template: ZeroPerf `Perf.md` §9.2; caveat detail: `BENCHMARK.md` §9. Two separate gaps, of different sizes: the automaticity mechanism (a `#[cfg(target_arch = "aarch64")]` gate, trivial once NEON code exists in Rust) vs. the NEON code itself, which needs a genuine translation from C `arm_neon.h` intrinsics to Rust `core::arch::aarch64` intrinsics — `blake2b_simd` has no `build.rs`/C-compilation hook to bolt an FFI wrapper onto, unlike `blake3`'s build-script-driven dispatch, so vendoring the C file directly is not a shortcut here; someone has to write the Rust translation |
...
| RT | **Unblocked (2026-07-15); zero Rust port code on disk yet** | Tromp's full multi-core `equi_miner.h`/`equi_miner.cpp` (real `pthread_barrier_t` round sync, `-t <nthreads>`). Previously recorded as genuinely blocked on `blake2-avx2/blake2bip.h`'s x86 AVX2 intrinsics with no ARM equivalent, with a VM explicitly not approved. Re-investigated: `clang -target x86_64-apple-macos -march=haswell` cross-compiles both the plain scalar build and the `-mavx2 -DNBLAKES=4` AVX2-interleaved build unmodified (after the same packed-struct header fix RK/RZ needed — `blake2sp_state`/`blake2bp_state` dropped, unused by this algorithm), and the resulting x86_64 binaries run correctly under Rosetta 2 (already present on this machine, no VM) — confirmed working at `(48,5)`: consistent solution counts at `-t 1/2/4`, and the AVX2 4-way-interleaved build runs and solves too. This resolves the blocker without a VM. No Rust port work has been done yet — this pass stopped at confirming the reference is buildable/runnable and reachable for cross-checking; the full port (cross-check harness, vector generation across every supported `(WN,RESTBITS)`, thread-count-invariance tests, then the actual Rust port with real multi-core structure per SOLVER_CORPUS.md's RT spec) is unstarted, well-scoped, unblocked follow-on work. **Strategy update 2026-07-16**: the reference binary builds *natively* on arm64 against the vendored modern BLAKE2b header (include-shim, no Rosetta) — verified trace-identical to the x86 original; Rosetta demoted to optional second oracle. `BLAKE/BLAKE.md` §0, `SOLVER_CORPUS.md` RT section |
...
RT's blocker was resolved the same session (cross-compile + Rosetta, no
VM) but the actual Rust port has not restarted yet — when it does, the
