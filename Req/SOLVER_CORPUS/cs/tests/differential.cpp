// Differential test: for every vector file in vectors/ (generated by
// running the actual Python reference, not a reimplementation -- see
// README.md "Validation"), re-solves with this C++ port and asserts the
// returned index vectors match exactly.
//
// Vector files are flat, simple JSON produced by this project's own
// generator script -- parsed here with a small hand-rolled reader
// rather than pulling in a JSON library dependency for a five-field
// schema. Not a general-purpose JSON parser; assumes the exact shape
// `{"n":N,"k":K,"nonce_hex":"...","solutions":[[i,i,...],...]}` (field
// order and whitespace exactly as this project's own generator emits).
...
// Skips whitespace, then expects and consumes the given literal.
...
int main(int argc, char** argv) {
    ...
    for (const auto& path : files) {
        ...
        cs::KListWagnerAlgorithm solver(v.n, v.k, hex_to_bytes(v.nonce_hex));
    ...
    if (failures > 0) {
        fprintf(stderr, "%d/%zu vector file(s) FAILED\n", failures, files.size());
        return 1;
    }
