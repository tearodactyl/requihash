...
int main(int argc, char** argv) {
    ...
    for (const auto& path : files) {
        ...
        cs_v6::KListWagnerAlgorithmV6 solver(v.n, v.k, hex_to_bytes(v.nonce_hex));
        auto solutions = solver.solve();

        // Set comparison, not sequence: this variant's bucket-walk order
        // (bucket-major, slot-minor) differs from the reference's
        // hash-table-iteration order, same reasoning as V5's differential
        // test -- see that file's comment for the full rationale.
