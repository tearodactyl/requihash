...
**F-A5.** The 2017 optimization plateau is explained: the solver family had reached the memory floor its data structure permits, so the plateau was completion, not stagnation. The Section 9.1 prototype premise — measuring GPU-feature headroom above the 2017 solvers — is superseded; the informative prototypes are now running the 2025 estimators against deployed parameters and benchmarking Requihash against Equihash on identical hardware [Reported]. Building the `Req/` solver independently re-derived the 2016-17 techniques from profiling: allocation (59%) and comparison sort (24%) dominate the naive solver, and the arena-plus-incomplete-bucket-sort fixes recover 1.86x — the same layout and sort techniques the original wave found, confirming they were the reachable wins and the memory floor was real [Measured].
...
Directions that survive these findings but exceed this document's scope.
