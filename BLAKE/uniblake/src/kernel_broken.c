/* uniblake compress kernel: `broken` — a deliberately-wrong kernel.
 *
 * Compiled ONLY under -DUB_ENABLE_BROKEN_KERNEL. Exists so a test can
 * prove the self-test gate (§5) actually rejects a variant that fails
 * to match the oracle — making the "forcing selects, never bypasses
 * the gate" guarantee observable rather than asserted. It is `ref`
 * with one round dropped, so its output diverges. Never registered in
 * a normal build.
 */
...
static void compress_one(struct ub_state *S, const uint8_t block[UB_BLOCKBYTES]) {
  ...
  ROUND(8);  ROUND(9);  ROUND(10); /* ROUND(11) intentionally dropped */
